...
## v17 - v16 Heuristic + Retrained Ensemble on New Dataset (Fixed Labels + Wider Model)

- Trains an 8-member ensemble ranker on top of the v16 heuristic using a new dataset generated by the v16 agent (auto-discover newest `data/*/candidates_v7.csv` from HF).
- Fixed training labels from `generate_training_data.py`: win/loss/tie branches replace the flat formula, cutting `positive_rate` from ~70% to ~35% and giving the model real win-vs-lose discrimination.
- Widened model architecture to 256 → 128 → 64 (was 160 → 80 → 40) for better capacity.
- Increased pair loss weight 1.05 → 1.10, max pairs per turn 12 → 14, patience 36 → 40 to exploit the cleaner label signal.
- Added `notebooks/v17/train_v17_ranker.py` and `notebooks/v17/v17_training_policy.ipynb`; cell 3 auto-discovers newest HF `data/*/candidates_v7.csv`. Artifacts upload to `devaanshpa/orbit-wars-agent/v17`.
