...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the v12 Orbit Wars supervised ensemble ranker.")
    ...
    parser.add_argument("--export-dir", default="notebooks/v12/exports")
    parser.add_argument("--epochs", type=int, default=int(os.environ.get("V12_EPOCHS", "280")))
    parser.add_argument("--batch-size", type=int, default=int(os.environ.get("V12_BATCH_SIZE", "4096")))
    parser.add_argument("--lr", type=float, default=float(os.environ.get("V12_LR", "0.00045")))
    parser.add_argument("--weight-decay", type=float, default=float(os.environ.get("V12_WEIGHT_DECAY", "0.00030")))
    parser.add_argument("--pair-loss-weight", type=float, default=float(os.environ.get("V12_PAIR_LOSS_WEIGHT", "1.05")))
    parser.add_argument("--max-pairs-per-turn", type=int, default=int(os.environ.get("V12_MAX_PAIRS_PER_TURN", "12")))
    parser.add_argument("--ensemble-size", type=int, default=int(os.environ.get("V12_ENSEMBLE_SIZE", "8")))
    parser.add_argument("--patience", type=int, default=int(os.environ.get("V12_PATIENCE", "36")))
    parser.add_argument("--dropout", type=float, default=float(os.environ.get("V12_DROPOUT", "0.13")))
    parser.add_argument("--score-scale", type=float, default=float(os.environ.get("V12_SCORE_SCALE", "205.0")))
    parser.add_argument("--seed", type=int, default=int(os.environ.get("V12_SEED", "907")))
    parser.add_argument("--checkpoint-every", type=int, default=int(os.environ.get("V12_CHECKPOINT_EVERY", "40")))
...
def find_training_csv(csv_arg, prefer_local=False):
    ...
    fixed = Path("notebooks/v12/data/candidates_v7.csv")
...
def save_member_checkpoint(model, best_state, nn, args, member_seed, epoch, best_valid_objective, checkpoint_context):
    ...
    if api is None:
        print(f"Saved local v12 checkpoint {checkpoint_path}", flush=True)
    ...
    try:
        api.upload_file(
            ...
            commit_message=f"Upload v12 checkpoint member_seed={member_seed} epoch={epoch}",
        )
        print(f"Uploaded v12 checkpoint to {remote_path}", flush=True)
    except Exception as exc:
        print(f"Failed to upload v12 checkpoint {checkpoint_path.name}: {exc}", flush=True)
    return checkpoint_path
...
def train(args):
    try:
        ...
        raise RuntimeError("PyTorch is required for v12 training. Install torch or run on Kaggle/Colab.") from exc
    ...
    (export_dir / "model_weights_v12.json").write_text(json.dumps(artifact, indent=2, sort_keys=True), encoding="utf-8")
    (export_dir / "feature_schema_v12.json").write_text(json.dumps({"features": feature_names}, indent=2), encoding="utf-8")
    (export_dir / "metrics_v12.json").write_text(json.dumps(metrics, indent=2, sort_keys=True), encoding="utf-8")
    (export_dir / "training_history_v12.json").write_text(json.dumps(histories, indent=2, sort_keys=True), encoding="utf-8")
    with (export_dir / "predictions_v12.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    try:
        ...
        plt.xlabel("epoch")
        plt.ylabel("loss")
        plt.title("v12 first member validation loss")
        plt.legend()
        plt.tight_layout()
        plt.savefig(graph_dir / "loss_curve_v12.png", dpi=150)
        plt.close()
        ...
        plt.ylabel("rows")
        plt.title("v12 ensemble prediction distribution")
        plt.legend()
        plt.tight_layout()
        plt.savefig(graph_dir / "prediction_histogram_v12.png", dpi=150)
        plt.close()
    except ModuleNotFoundError:
        print("matplotlib is not installed; skipped graph generation.", flush=True)

    print(json.dumps(metrics, indent=2, sort_keys=True), flush=True)
    print(f"Saved v12 model artifact: {export_dir / 'model_weights_v12.json'}", flush=True)
    ...
    if args.upload and hf_api is not None:
        hf_api.upload_folder(
            ...
            commit_message="Upload v12 Orbit Wars supervised ensemble ranker artifacts and graphs",
        )
