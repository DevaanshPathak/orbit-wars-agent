...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the v7 Orbit Wars counterfactual ensemble ranker.")
    parser.add_argument("--csv", default=os.environ.get("CANDIDATES_CSV", ""))
    parser.add_argument("--export-dir", default="notebooks/v7/exports")
    parser.add_argument("--epochs", type=int, default=int(os.environ.get("V7_EPOCHS", "160")))
    parser.add_argument("--batch-size", type=int, default=int(os.environ.get("V7_BATCH_SIZE", "2048")))
    parser.add_argument("--lr", type=float, default=float(os.environ.get("V7_LR", "0.00055")))
    parser.add_argument("--weight-decay", type=float, default=float(os.environ.get("V7_WEIGHT_DECAY", "0.00025")))
    parser.add_argument("--pair-loss-weight", type=float, default=float(os.environ.get("V7_PAIR_LOSS_WEIGHT", "0.80")))
    parser.add_argument("--max-pairs-per-turn", type=int, default=9)
    parser.add_argument("--ensemble-size", type=int, default=int(os.environ.get("V7_ENSEMBLE_SIZE", "4")))
    parser.add_argument("--patience", type=int, default=int(os.environ.get("V7_PATIENCE", "24")))
...
def find_training_csv(csv_arg):
    ...
    fixed = Path("notebooks/v7/data/candidates_v7.csv")
    if fixed.exists():
        return fixed
...
def train_member(torch, nn, functional, args, member_seed, tensors, pairs, feature_count, eval_context):
...
def train(args):
    try:
        ...
        raise RuntimeError("PyTorch is required for v7 training. Install torch or run on Kaggle/Colab.") from exc
    ...
    tuned_blend = tune_blend(rows, all_probs, positive_mask, valid_indices, 190.0)
    ...
    (export_dir / "model_weights_v7.json").write_text(json.dumps(artifact, indent=2, sort_keys=True), encoding="utf-8")
    (export_dir / "feature_schema_v7.json").write_text(json.dumps({"features": feature_names}, indent=2), encoding="utf-8")
    (export_dir / "metrics_v7.json").write_text(json.dumps(metrics, indent=2, sort_keys=True), encoding="utf-8")
    (export_dir / "training_history_v7.json").write_text(json.dumps(histories, indent=2, sort_keys=True), encoding="utf-8")
    with (export_dir / "predictions_v7.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    try:
        ...
        plt.xlabel("epoch")
        plt.ylabel("loss")
        plt.title("v7 first member validation loss")
        plt.legend()
        plt.tight_layout()
        plt.savefig(graph_dir / "loss_curve_v7.png", dpi=150)
        plt.close()
        ...
        plt.ylabel("rows")
        plt.title("v7 ensemble prediction distribution")
        plt.legend()
        plt.tight_layout()
        plt.savefig(graph_dir / "prediction_histogram_v7.png", dpi=150)
        plt.close()
    except ModuleNotFoundError:
        print("matplotlib is not installed; skipped graph generation.", flush=True)

    print(json.dumps(metrics, indent=2, sort_keys=True), flush=True)
    print(f"Saved v7 model artifact: {export_dir / 'model_weights_v7.json'}", flush=True)
    ...
    if args.upload:
        ...
        api.upload_folder(
            ...
            commit_message="Upload v7 Orbit Wars ensemble ranker artifacts and graphs",
        )
