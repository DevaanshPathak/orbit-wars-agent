...
HF_REPO_TYPE = "model"
SFT_REMOTE_PREFIX = "v18/sft"
GRPO_REMOTE_PREFIX = "v18/grpo"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run v18 constrained GRPO-style policy improvement over Orbit Wars candidate groups."
    )
    parser.add_argument("--csv", default=os.environ.get("CANDIDATES_CSV", ""))
    parser.add_argument(
        "--data-remote-path",
        default=os.environ.get("V18_GRPO_DATA_REMOTE_PATH", PINNED_DATASET),
    ...
    parser.add_argument(
        "--sft-artifact",
        default=os.environ.get("V18_SFT_ARTIFACT", ""),
        help="Optional local SFT JSON override. By default GRPO downloads the SFT artifact from Hugging Face.",
    )
    parser.add_argument(
        "--sft-remote-path",
        default=os.environ.get("V18_SFT_REMOTE_PATH", f"{SFT_REMOTE_PREFIX}/model_weights_v18_sft.json"),
        help="Hugging Face repo path for the SFT artifact used by GRPO.",
    )
    parser.add_argument(
        "--prefer-local-sft",
        action="store_true",
        help="Use notebooks/v18/exports/sft/model_weights_v18_sft.json before trying Hugging Face.",
    )
    parser.add_argument("--export-dir", default="notebooks/v18/exports/grpo")
    parser.add_argument("--epochs", type=int, default=int(os.environ.get("V18_GRPO_EPOCHS", "160")))
    parser.add_argument("--batch-groups", type=int, default=int(os.environ.get("V18_GRPO_BATCH_GROUPS", "192")))
    parser.add_argument("--samples-per-group", type=int, default=int(os.environ.get("V18_GRPO_SAMPLES", "12")))
    parser.add_argument("--temperature", type=float, default=float(os.environ.get("V18_GRPO_TEMPERATURE", "0.85")))
    parser.add_argument("--lr", type=float, default=float(os.environ.get("V18_GRPO_LR", "0.00018")))
    parser.add_argument("--weight-decay", type=float, default=float(os.environ.get("V18_GRPO_WEIGHT_DECAY", "0.00018")))
    parser.add_argument("--kl-weight", type=float, default=float(os.environ.get("V18_GRPO_KL_WEIGHT", "0.10")))
    parser.add_argument("--entropy-weight", type=float, default=float(os.environ.get("V18_GRPO_ENTROPY_WEIGHT", "0.018")))
    parser.add_argument("--supervised-anchor", type=float, default=float(os.environ.get("V18_GRPO_SUPERVISED_ANCHOR", "0.22")))
    parser.add_argument("--patience", type=int, default=int(os.environ.get("V18_GRPO_PATIENCE", "36")))
    parser.add_argument("--checkpoint-every", type=int, default=int(os.environ.get("V18_GRPO_CHECKPOINT_EVERY", "30")))
    parser.add_argument(
        "--device",
...
def find_training_csv(csv_arg, remote_path="", prefer_local=False, repo_id=HF_REPO_ID, repo_type=HF_REPO_TYPE):
    ...
    candidates = []
    if root.exists():
        candidates.extend(root.glob("*/candidates_v18.csv"))
...
def find_sft_artifact(path_arg, repo_id, repo_type, remote_path, prefer_local=False):
    ...
    local = Path("notebooks/v18/exports/sft/model_weights_v18_sft.json")
...
def candidate_reward_components(row, label, selected, counterfactual):
    ...
    This reward intentionally blends several bounded signals instead of a single
    final score or selected-candidate flag. The selected bonus is kept small so
    GRPO cannot win by simply cloning the heuristic. Failure and crash features
    are explicit penalties so high apparent reward cannot hide unsafe moves.
    """
    components = {
        "label_signal": (label - 0.5) * 1.15,
        "advantage_delta_15": clamp(row_float(row, "future_advantage_delta_15", 0.0) / 85.0, -1.25, 1.25),
        "advantage_delta_30": clamp(row_float(row, "future_advantage_delta_30", 0.0) / 140.0, -0.90, 0.90) * 0.45,
        "production_delta": clamp(row_float(row, "future_production_delta_15", 0.0) / 9.0, -0.75, 0.75) * 0.55,
        "planet_delta": clamp(row_float(row, "future_planet_delta_15", 0.0) / 3.0, -0.75, 0.75) * 0.45,
...
def save_grpo_checkpoint(args, torch, nn, model, epoch, item, history, feature_names, means, scales, sft_path, blend):
    ...
    print(f"Saved v18 GRPO checkpoint: {path}", flush=True)
    maybe_upload_file(
        args,
        path,
        f"{GRPO_REMOTE_PREFIX}/checkpoints/{path.name}",
        f"Upload v18 GRPO checkpoint epoch {epoch}",
    )
...
def train(args):
    try:
        ...
        raise RuntimeError("PyTorch is required for v18 GRPO training. Install torch or use Kaggle/Colab.") from exc
    ...
    (export_dir / "model_weights_v18_grpo.json").write_text(json.dumps(artifact, indent=2, sort_keys=True), encoding="utf-8")
    (export_dir / "metrics_v18_grpo.json").write_text(json.dumps(metrics, indent=2, sort_keys=True), encoding="utf-8")
    (export_dir / "training_history_v18_grpo.json").write_text(json.dumps(history, indent=2, sort_keys=True), encoding="utf-8")
    with (export_dir / "predictions_v18_grpo.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    try:
        ...
        plt.xlabel("epoch")
        plt.title("v18 GRPO diagnostics")
        plt.legend()
        plt.tight_layout()
        plt.savefig(graph_dir / "grpo_diagnostics_v18.png", dpi=150)
        plt.close()
    except ModuleNotFoundError:
        print("matplotlib is not installed; skipped graph generation.", flush=True)

    print(json.dumps(metrics, indent=2, sort_keys=True), flush=True)
    print(f"Saved v18 GRPO artifact: {export_dir / 'model_weights_v18_grpo.json'}", flush=True)
    ...
    if args.upload:
        ...
        api.upload_folder(
            ...
            commit_message="Upload v18 GRPO Orbit Wars policy artifacts and graphs",
        )
