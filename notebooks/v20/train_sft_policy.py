...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train the v20 conservative counterfactual-weighted SFT candidate policy from Orbit Wars candidate groups."
    )
    parser.add_argument("--csv", default=os.environ.get("CANDIDATES_CSV", ""))
    parser.add_argument(
        "--data-remote-path",
        default=os.environ.get("V20_SFT_DATA_REMOTE_PATH", PINNED_DATASET),
        help="Hugging Face repo path for candidates_v20.csv or candidates_v19.csv.",
    )
    parser.add_argument(
        "--prefer-local-data",
        action="store_true",
        help="Use a local candidates_v20.csv or candidates_v19.csv before trying Hugging Face.",
    )
    parser.add_argument("--export-dir", default="notebooks/v20/exports/sft")
    parser.add_argument("--epochs", type=int, default=int(os.environ.get("V20_SFT_EPOCHS", "180")))
    parser.add_argument("--batch-groups", type=int, default=int(os.environ.get("V20_SFT_BATCH_GROUPS", "256")))
    parser.add_argument("--lr", type=float, default=float(os.environ.get("V20_SFT_LR", "0.00055")))
    parser.add_argument("--weight-decay", type=float, default=float(os.environ.get("V20_SFT_WEIGHT_DECAY", "0.00028")))
    parser.add_argument("--dropout", type=float, default=float(os.environ.get("V20_SFT_DROPOUT", "0.12")))
    parser.add_argument("--bce-weight", type=float, default=float(os.environ.get("V20_SFT_BCE_WEIGHT", "0.24")))
    parser.add_argument("--pair-weight", type=float, default=float(os.environ.get("V20_SFT_PAIR_WEIGHT", "0.30")))
    parser.add_argument("--rank-weight", type=float, default=float(os.environ.get("V20_SFT_RANK_WEIGHT", "0.55")))
    parser.add_argument("--ensemble-size", type=int, default=int(os.environ.get("V20_SFT_ENSEMBLE_SIZE", "4")))
    parser.add_argument("--patience", type=int, default=int(os.environ.get("V20_SFT_PATIENCE", "34")))
    parser.add_argument("--checkpoint-every", type=int, default=int(os.environ.get("V20_SFT_CHECKPOINT_EVERY", "30")))
    parser.add_argument(
        "--multi-gpu",
        action="store_true",
        default=os.environ.get("V20_SFT_MULTI_GPU", "1").strip().lower() not in {"0", "false", "no", "off"},
        help="Use torch.nn.DataParallel when multiple CUDA devices are available. Enabled by default.",
    )
    parser.add_argument(
        "--device",
        choices=("cuda", "cpu", "auto"),
        default=os.environ.get("V20_DEVICE", "cuda"),
...
def find_training_csv(csv_arg, remote_path="", prefer_local=False):
    ...
    local_candidates = []
    for pattern in (
        "notebooks/v20/data/candidates_v20.csv",
        "notebooks/v20/data/candidates_v19.csv",
    ):
        path = Path(pattern)
        if path.exists():
            return path
    ...
    root = Path("data")
    if root.exists():
        local_candidates.extend(root.glob("*/candidates_v20.csv"))
...
def target_distribution(rows, labels, selected, counterfactual, group):
    """v20: counterfactual-weighted target distribution.
    ...
    for raw_index in group:
        ...
        # v20: blend counterfactual rollout delta into target weight
        ...
        if counterfactual[raw_index]:
            value += 0.25
        # v20: penalize crash candidates more aggressively
...
def save_sft_checkpoint(args, model, nn, member_seed, epoch, item, history, feature_names, means, scales):
    ...
    print(f"Saved v20 SFT checkpoint: {path}", flush=True)
    maybe_upload_file(
        args,
        path,
        f"{HF_REMOTE_PREFIX}/checkpoints/{path.name}",
        f"Upload v20 SFT checkpoint seed {member_seed} epoch {epoch}",
    )
...
def train(args):
    try:
        ...
        raise RuntimeError("PyTorch is required for v20 SFT training. Install torch or use Kaggle/Colab.") from exc
    ...
    (export_dir / "model_weights_v20_sft.json").write_text(json.dumps(artifact, indent=2, sort_keys=True), encoding="utf-8")
    (export_dir / "feature_schema_v20_sft.json").write_text(json.dumps({"features": feature_names}, indent=2), encoding="utf-8")
    (export_dir / "metrics_v20_sft.json").write_text(json.dumps(metrics, indent=2, sort_keys=True), encoding="utf-8")
    (export_dir / "training_history_v20_sft.json").write_text(json.dumps(histories, indent=2, sort_keys=True), encoding="utf-8")
    with (export_dir / "predictions_v20_sft.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    try:
        ...
        plt.xlabel("epoch")
        plt.title("v20 SFT validation")
        plt.legend()
        plt.tight_layout()
        plt.savefig(graph_dir / "sft_validation_v20.png", dpi=150)
        plt.close()
        ...
        plt.ylabel("rows")
        plt.title("v20 SFT prediction distribution")
        plt.legend()
        plt.tight_layout()
        plt.savefig(graph_dir / "sft_prediction_histogram_v20.png", dpi=150)
        plt.close()
    except ModuleNotFoundError:
        print("matplotlib is not installed; skipped graph generation.", flush=True)

    print(json.dumps(metrics, indent=2, sort_keys=True), flush=True)
    print(f"Saved v20 SFT artifact: {export_dir / 'model_weights_v20_sft.json'}", flush=True)
    ...
    if args.upload:
        ...
        api.upload_folder(
            ...
            commit_message="Upload v20 SFT Orbit Wars policy artifacts and graphs",
        )
